...
// RightPath is added to the boss enemy at runtime so it moves from left to right across the screen.
public class RightPath : MonoBehaviour
{
    ...
    void Update()
    {
        // Translate (move) the boss enemy - the direction is right and the amount depends on speed and elapsed time.
        // ALL animations should factor in time for consistency across different hardware!!!
        transform.Translate(Vector2.right * speed * Time.deltaTime);

        // checkEndOfLife is a function that check's the boss's position and removed it from the scene when it reaches the right edge of the screen.
    ...
    void checkEndOfLife()
    {
        // If the boss's x position is greater than or equal to 8 then remove it from the scene by calling Destroy.
